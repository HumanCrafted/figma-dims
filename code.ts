 ...
 */
async function createDimensionRoot(orient: 'H' | 'V', labelStyle: LabelStyle, flip: boolean): Promise<FrameNode> {
  const s = settings;
  ...
  if (inline) await assembleInline(root, orient, flip, s, label);
  else assembleStandard(root, orient, flip, s, label);
 ...
 */
async function updateDimension(old: FrameNode): Promise<FrameNode | null> {
  if (old.getPluginData(K.isDim) !== '1') return null;
  const orient: 'H' | 'V' = old.getPluginData(K.orient) === 'V' ? 'V' : 'H';
  const labelStyle: LabelStyle = old.getPluginData(K.labelStyle) === 'inline' ? 'inline' : 'standard';
  ...
  if (parent && idx >= 0) parent.insertChild(idx, root); // same parent + z-order
  root.x = x;
  root.y = y;
  old.remove();
...
figma.ui.onmessage = async (msg) => {
  if (msg.type === 'create') {
    ...
    const dims = figma.currentPage.findAll((n) => n.getPluginData(K.isDim) === '1') as SceneNode[];
    ...
    const sel = figma.currentPage.selection.filter(
      (n) => typeof n.getPluginData === 'function' && n.getPluginData(K.isDim) === '1'
    ) as FrameNode[];
    if (!sel.length) {
