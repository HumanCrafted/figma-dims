 ...
 */
async function inlineSegment(orient, thickness, cap, capOnStart) {
    const v = figma.createVector();
 ...
 */
function featurePad(root, orient, flip, gap) {
    if (orient === 'H') {
        if (flip)
            root.paddingTop = gap;
        else
            root.paddingBottom = gap;
    }
    else {
        if (flip)
            root.paddingRight = gap;
        else
            root.paddingLeft = gap;
    }
}
/**
 ...
 */
function assembleStandard(root, orient, flip, s, label) {
...
figma.ui.onmessage = async (msg) => {
    if (msg.type === 'create') {
        ...
        if (msg.labelStyle === 'standard' || msg.labelStyle === 'inline')
            settings.labelStyle = msg.labelStyle;
        if (typeof msg.flip === 'boolean')
            settings.flip = msg.flip;
